...
class V1 extends Model{  
    ...
    static $data          =   [];
    //化妆品功效
    static $effects       =   ['美白祛斑','保湿','抗敏舒缓','去角质','去黑头','抗痘'];
    //用户信息
    static $userInfo      =   [];
    //错误码
    static $ERROR         =   array(
    ...
    public function actionIndex(){
        ...
        //解密
        // parse_str(Functions::passport_decrypt($parameter['token'], self::$key), $data);
        //无数据 
        if (empty($data))
        {
    ...
    public function actionMixed() {
        return $this->renderPartial('mixed');
    }
    /**
    ...
    PUBLIC function index() {
        ...
        $sql    = "SELECT id,title,article_img,like_num,comment_num,created_at FROM {{%article}} WHERE status = '1' ORDER BY  id DESC LIMIT 20";
        ...
        if(!empty($return['article'])){
            foreach ($return['article'] as $key => $value) {
                ...
                $return['article'][$key]['linkUrl']      = Yii::$app->params['mfrontendUrl'].'article/index?id='.$value['id'].'&isNew=0';
    ...
    PUBLIC function articleList() {
        ...
        $sql    = "SELECT id,title,article_img,like_num,comment_num,created_at FROM {{%article}} WHERE status = '1' ORDER BY  id DESC LIMIT $pageMin,$pageSize";
        ...
        if(!empty($rows)){
            foreach ($rows as $key => $value) {
                ...
                $rows[$key]['linkUrl']      = Yii::$app->params['mfrontendUrl'].'article/index?id='.$value['id'].'&isNew=0';
    ...
    PUBLIC function articleInfo() {
        ...
        if(!empty($rows)){
            ...
            $rows['linkUrl']      = Yii::$app->params['mfrontendUrl'].'article/index?id='.$rows['id'].'&isNew=0';
    ...
    PUBLIC function commentList() {
        ...
        $sql    = "SELECT C.id,C.user_id,C.comment,C.like_num,U.username,U.img,U.birth_year,S.dry,S.tolerance,S.pigment,S.compact FROM {{%comment}} C 
                   LEFT JOIN {{%user}} U ON C.user_id = U.id 
                   LEFT JOIN {{%user_skin}} S ON C.user_id = S.uid
    ...
    */
    PUBLIC function brandList(){
        ...
        $action         = self::$data['from'].'CosmeticsList';

        $cosmeticsList  = Cosmetics::$action(); 
    ...
    */
    PUBLIC function addRemind(){
        ...
        $isSeal      = intval(self::$data['is_seal']);
    ...
    */
    PUBLIC function userGrass(){
        ...
        $effect    = isset(self::$data['effect'])   ? Functions::checkStr(self::$data['effect']) :  ''  ;
        ...
        $whereStr  .= $effect ? " AND M.component_action like '%$effect%'" : '' ;

        /*分页*/
        $sql        = " SELECT COUNT(*) AS num FROM (SELECT C.relation_id FROM {{%user_collect}} C  
                        LEFT JOIN {{%product_details}} D ON C.relation_id = D.id
                        LEFT JOIN {{%product_relate}} R ON  D.id = R.product_id 
                        LEFT JOIN {{%product_component}} M ON  R.component_id = M.id
                        WHERE $whereStr GROUP BY C.relation_id) t";
        ...
        $selectSql  = " SELECT D.id,D.product_name,D.product_img FROM {{%user_collect}} C  
                        LEFT JOIN {{%product_details}} D ON C.relation_id = D.id
                        LEFT JOIN {{%product_relate}} R ON  C.relation_id = R.product_id 
                        LEFT JOIN {{%product_component}} M ON  R.component_id = M.id
                        WHERE $whereStr GROUP BY C.relation_id ORDER BY C.add_time DESC  LIMIT $pageMin,$pageSize";
        ...
        if($return['status'] == 1){
            ...
            if($userInfo){
                foreach ($userInfo as $key => $value) {
                    $userSkinArr[$value['type']]['name']       = $value['name'];
                    $userSkinArr[$value['type']]['letter']     = $value['letter'];
                }
            }
            $userSkinArr = array_values($userSkinArr);
            $data  = ['status' => 1, 'msg' =>self::$ERROR['1'],'isComplete' => $isComplete,'addMoney' => $addMoney,'userSkin'=>$userSkinArr];
        }else{
             $data = ['status' => -1, 'msg' =>self::$ERROR['0']];
        }
        return  json_encode($data);
    ...
    */
    ...
    PUBLIC function userCollect(){
        ...
        foreach ($data as $key => $value) {
            ...
            $data[$key]['linkUrl']      = Yii::$app->params['mfrontendUrl'].'article/index?id='.$value['id'].'&isNew=0';
        }
        ...
        if(!empty($result)){
            ...
            if(!empty($article_ids)){
                ...
                foreach ($article_info as $k2 => $v2) {
                    ...
                    $article_info_new[$v2['id']]['linkUrl'] = Yii::$app->params['mfrontendUrl'].'article/index?id='.$v2['id'].'&isNew=0';
    ...
    */
    ...
    {   
        $requiredParameter  = array('user_id','attribute','content');
        $this->completeParameter($requiredParameter);
    ...
    **/
    PUBLIC function userProduct(){
        ...
        $userId     = !empty(self::$data['user_id'])         ? intval(self::$data['user_id']) : 0 ;
        $pageSize   = !empty(self::$data['pageSize'])     ? intval(self::$data['pageSize']) : 10;
        $page       = !empty(self::$data['page'])         ? intval(self::$data['page']) : 1;

        $count_sql  = "SELECT COUNT(*) AS num FROM {{%user_product}}  WHERE user_id = '$userId'";
        ...
        if($num > 0){
            ...
            $selectSql      = "SELECT * FROM {{%user_product}}  WHERE user_id = '$userId' ORDER BY add_time DESC LIMIT $pageMin,$pageSize";
            $result         = Yii::$app->db->createCommand($selectSql)->queryAll();
        }
        //消息设为已读
        $readSql = "UPDATE {{%notice_user}} SET status = '1' WHERE user_id = '$userId' AND type = '2'";
    ...
    */
    PUBLIC function searchAssociate(){
        ...
        $productList = $data['msg']['product'];
        foreach ($productList as $key => $value) {
            $productList[$key]['type'] = 1;
            $productList[$key]['name'] = $value['product_name'];
            unset($productList[$key]['product_name']);
        }
        $newData = ['status' => 1,'msg' => $productList];
    ...
    */
    PUBLIC function searchQuery(){
        ...
        $effect     = !empty(self::$data['effect'])     ? Functions::checkStr(self::$data['effect']) :   '' ;
        $pageSize   = !empty(self::$data['pageSize'])   ? intval(self::$data['pageSize']) : 20;
        $page       = !empty(self::$data['page'])       ? intval(self::$data['page']) : 1;

        $effectId   = 0;
        if($effect){
            foreach (self::$effects as $key => $value) {
                if($value == $effect){
                    $effectId = $key + 1;
                }
            }
        ...
        //产品列表
        $data = Skin::searchProduct($param);
        $data['msg']['data']          = $data['msg']['product'];
        $data['msg']['categories']    = Functions::categoryList();
        $data['msg']['effects']       = self::$effects;
        unset($data['msg']['product']);
        //存入热搜表
        if(!empty($keyword)){
