...
class V123 extends Model{
    ...
    PUBLIC function index() {
        ...
        //查询菜单
        $return['menu'] = Functions::getAppMenu();
        //一级文章栏目
        $firstGory   = [];
        ...
        //视频列表
        $return['videosList']   = Functions::indexVideosList($param);
        //首页文章
    ...
    PUBLIC function videosList(){
        ...
        //视频列表
        $list       = Functions::getVideosList($param);
        $data       = ['status' => 1, 'msg' => $list['data'],'pageTotal' => $list['num'], 'pageSize'=> $pageSize];
        return  json_encode($data);
    }
    /**
    ...
    PUBLIC function articleList() {
        ...
        //先搜索字符
        if($brandId){
            $sql    = "SELECT name,ename  FROM {{%brand}} WHERE id = '$brandId'";
            $nameArr   = Yii::$app->db->createCommand($sql)->queryOne();
            if(empty($nameArr) || (!$nameArr['name'] && !$nameArr['ename']) ){
                return json_encode(['status' => -19, 'msg' => self::$ERROR[-19]]);
            }
            $query       = new Query();
            $name        = $nameArr['name'] ? $nameArr['name'] : $nameArr['ename'];
        ...
        if($page == 1){
            ...
            $sql        = " SELECT id,title,click_num,article_img,like_num,comment_num,created_at 
                            ...
                            WHERE $whereStr 
                            ORDER BY $orderBy";
            $topData   = Yii::$app->db->createCommand($sql)->queryAll();

            if(!empty($topData)){
                foreach ($topData as $k => $v) {
                    $article = $v;
                    $article['article_img']  = Functions::get_image_path($v['article_img'],1,247,154);
                    $article['created_at']   = Tools::HtmlDate($v['created_at']);
                    $article['linkUrl']      = Yii::$app->params['mfrontendUrl'].'article/index?id='.$v['id'].'&isNew=0&from='.self::$data['from'];
                    $article['isGras']       = Functions::userIsGras($uid,$v['id'],2) ? 1 : 0;
                    $data[] = $article;
                    unset($article);
                }
            }
        ...
        if(!empty($rows)){
            foreach ($rows as $key => $value) {
                $article = $value;
                $article['article_img']  = Functions::get_image_path($value['article_img'],1,247,154);
                $article['created_at']   = Tools::HtmlDate($value['created_at']);
                $article['linkUrl']      = Yii::$app->params['mfrontendUrl'].'article/index?id='.$value['id'].'&isNew=0&from='.self::$data['from'];
                $article['isGras']       = Functions::userIsGras($uid,$value['id'],2) ? 1 : 0;
                $data[] = $article;
                unset($article);
            }
        }
    ...
    PUBLIC function articleInfo() {
        ...
        if(!empty($commentList)){
            foreach ($commentList as $key => $value) {
                $commentArr[$key]   =  Functions::getCommentInfo($value['id'],$uid,1);
    ...
    PUBLIC function productList() {
        ...
        $sql    = "SELECT P.id,B.id AS brand_id,B.name AS brand_name,B.rule,P.is_top,P.product_name,P.product_img,P.price,P.form,P.star,B.name AS brand_name
    ...
    PUBLIC function productInfo() {
        ...
        if($uid){
            ...
            $userProduct= Functions::getNewUserProduct($uid);
            $userProduct= $userProduct ? $userProduct : [];
        }
        $rows['isGras']         = $isGras;
        $rows['userProduct']    = $userProduct;
        ...
        $extend_sql     = "SELECT type,url,link_price,tb_goods_id FROM {{%product_link}} WHERE product_id = {$id} AND status = 1";
        ...
        //获取商品的优惠券
        $date = date('Y-m-d');
        $bonusSql = "SELECT product_id, price, bonus_link FROM {{%product_bonus}} 
                     WHERE end_date >= {$date} AND is_off = 1 AND product_id = {$id}";
        $bonus = Yii::$app->db->createCommand($bonusSql)->queryOne();
        $bonus AND $rows['bonus'] = $bonus;
    ...
    PUBLIC function commentList() {
        ...
        if(!empty($commentList)){
            foreach ($commentList as $key => $value) {
                ...
                $rows[$key]    = Functions::getCommentInfo($value['id'],$uid,1);
                $reply         = Functions::getCommentReply($value['id']);
    ...
    PUBLIC function commentInfo() {
        ...
        $commentInfo= Functions::getCommentInfo($id,$uid,1);
        ...
        switch ($commentInfo['type']) {
            ...
            case '2':
                ...
                break;
            case '3':
                $videoSql = "SELECT title FROM {{%video}} WHERE id = {$commentInfo['post_id']}";
                $title = Yii::$app->db->createCommand($videoSql)->queryScalar();
                $commentInfo['title'] = $title ? $title : '';
                break;
            case '4':
                $postSql = "SELECT T.title FROM {{%post}} P LEFT JOIN {{%topic}} T ON P.topic_id = T.id WHERE P.id = {$commentInfo['post_id']}";
                $title   = Yii::$app->db->createCommand($postSql)->queryScalar();
                $commentInfo['title'] = $title ? $title : '';
                break;
            default:
                $commentInfo['title'] = '';
    ...
    PUBLIC function addCommentLike()
    {
        ...
        if($isExsit){
            ...
            if($userId != $commentInfo['user_id']) {
                //点赞消息
                ReplyFunctions::reply($userId,$commentId,ReplyFunctions::$REPLY_LIKE_COMMENT,'','',$commentId);
            }
        }
    ...
    */
    PUBLIC function addComment(){
        ...
        $attachment = isset(self::$data['attachment']) ? self::$data['attachment'] : '';
        $referer  = strtolower(self::$data['from']) == 'ios' ? 'ios' : 'android';
        $time     = time();
        $firstId  = 0;
        ...
        switch ($type) {
            ...
            case '4':
                ...
                break;
            default:
                $data = ['status' => -200, 'msg' => self::$ERROR['-200']];
                return  json_encode($data);
                break;
        }
        ...
        if($parentId && $newcommentId){
            //二级评论消息
            ReplyFunctions::reply($userId,$parentId,ReplyFunctions::$REPLY_OTHER_COMMENT,$comment,'',$newcommentId);
        }elseif($type == 4){
            ReplyFunctions::reply($userId,$id,ReplyFunctions::$REPLY_POST,$comment);
        }
        ...
        if($return){
            ...
            //入库附件
            if($attachment){
                foreach ($attachment as $attachKey => $attachVal) {
                    $attachmentSql  = "INSERT INTO {{%attachment}} (cid,uid,attachment,dateline) VALUES ('$newcommentId','$userId','$attachVal','$time')";
                    Yii::$app->db->createCommand($attachmentSql)->execute();
                    $attachmentList[] =  Functions::get_image_path($attachVal);
                    unset($attachmentSql);
                }
            ...
            //返回图片地址，失了智了
            $data = ['status' => 1, 'msg' => ['commentId' => $newcommentId,'attachment' => $attachmentList,'flag' => $addMoney,'content'=>$content]];
        }else{
    ...
    */
    PUBLIC function addRemind(){
        //验证
        $requiredParameter  = array('product','overdue_time');
        $this->completeParameter($requiredParameter);

        $userId      = intval(self::$data['user_id']);
        $brandId     = intval(self::$data['brand_id']);
        $product_id  = isset(self::$data['product_id']) ? intval(self::$data['product_id']) : 0 ;
        ...
        $qualityTime   = isset(self::$data['quality_time']) ? intval(self::$data['quality_time']) : 0 ;
        $days           = isset(self::$data['days']) ? intval(self::$data['days']) : 0 ;
        $sealTime    = intval(self::$data['seal_time']);
        $overdueTime = intval(self::$data['overdue_time']);
        $up_id      = isset(self::$data['up_id']) ? intval(self::$data['up_id']) : 0 ;
        ...
        if($qualityTime){
            $time_str = "+$qualityTime month";
        }else{
            $time_str = "+$days days";
        }
        $expire_time = $isSeal ? strtotime(date('Y-m-d',strtotime($time_str,$sealTime))) : $overdueTime;
        $expire_time = $expire_time > $overdueTime ? $overdueTime : $expire_time;
        if(!empty($up_id)){
            $sql  = "UPDATE {{%user_product}} SET user_id='$userId',brand_id='$brandId',brand_name='$brandName',product='$product',img='$newProductImg',is_seal='$isSeal',seal_time='$sealTime',quality_time='$qualityTime',overdue_time='$overdueTime',expire_time='$expire_time',add_time='$time',days='$days',product_id='$product_id' WHERE id = '$up_id'";
        }else{
            //入库
            $sql  = "INSERT INTO {{%user_product}} (user_id,brand_id,brand_name,product,img,is_seal,seal_time,quality_time,overdue_time,expire_time,add_time,days,product_id) 
                     VALUES ('$userId','$brandId','$brandName','$product','$newProductImg','$isSeal','$sealTime','$qualityTime','$overdueTime','$expire_time','$time','$days','$product_id')";
        }
    ...
    */
    PUBLIC function userGrass(){
        ...
        foreach ($return['data'] as $key => $value) {
            ...
            //获取商品的优惠券信息
            $sql  = " SELECT bonus_link, price FROM {{%product_bonus}} WHERE product_id = {$value['id']} AND end_date >= '{$date}'";
            $coupon = Yii::$app->db->createCommand($sql)->queryOne();

            $return['data'][$key]['coupon_price'] = !empty($coupon) ? $coupon['price'] : 0;
            $return['data'][$key]['coupon_link'] = !empty($coupon) ? $coupon['bonus_link'] : 0;
        }
    ...
    */
    ...
    PUBLIC function userComment(){
        ...
        $whereStr   = "status = '1' AND user_id = '$userId' AND parent_id = '0'";
        $whereStr   .= $type ? " AND type = '$type' " : '';
        $whereStr   .= $type==4 ? " AND type = '4' " : " AND type != '4' ";
        $sql        = "SELECT COUNT(*) AS num FROM {{%comment}}  WHERE $whereStr";
        $num        = Yii::$app->db->createCommand($sql)->queryScalar();

        $selectSql  = "SELECT id,type,post_id,comment,star,created_at FROM {{%comment}}  WHERE $whereStr ORDER BY created_at DESC LIMIT $pageMin,$pageSize";
        ...
        if(!empty($result)){
            foreach ($result as $key => $value) {
                switch ($value['type']) {
                    case '1':
                        $product_ids[] = $value['post_id'];
                        break;
                     case '2':
                        $article_ids[] = $value['post_id'];
                        break;
                    case '3':
                        $video_ids[]   = $value['post_id'];
                        break;
                }
            }
            ...
            if(!empty($video_ids)){
                $video_str      = Functions::db_create_in($video_ids,'id');
                $videoSql       = "SELECT id,title as name,thumb_img FROM {{%video}}  WHERE $video_str";
                $video_info     = Yii::$app->db->createCommand($videoSql)->queryAll();
                foreach ($video_info as $k3 => $v3) {
                    $video_info_new[$v3['id']]            = $v3;
                    $video_info_new[$v3['id']]['img']     = Functions::get_image_path($v3['thumb_img'],1);
                    unset($video_info_new[$v3['id']]['thumb_img']);
                }
            }
            $orderArr = [];
            foreach ($result as $k3 => $v3) {
                switch ($v3['type']) {
                    case '1':
                        $info = $product_info_new;
                        break;
                    case '2':
                        $info = $article_info_new;
                        break;
                    case '3':
                        $info = $video_info_new;
                        break;
                }
                $orderArr[$k3] = intval($v3['created_at']);
        ...
        if(empty($type) || $type==4){
            $postSql       = "SELECT P.id,P.content as post_title,P.created_at,T.title as topic_title FROM {{%post}} P
                            LEFT JOIN {{%topic}} T ON P.topic_id = T.id
                            WHERE user_id = '$userId'";
            $post_info     = Yii::$app->db->createCommand($postSql)->queryAll();
            if(!empty($post_info)){
                foreach ($post_info as $k4 => $v4) {
                    $postArr['type']                 = '4';
                    $postArr['created_at']           = $v4['created_at'];
                    $postArr['post_id']              = $v4['id'];
                    $imgArr                          = Functions::getPostAttachment($v4['id']);
                    $postArr['detail']               = $v4;
                    $postArr['detail']['post_img']   = $imgArr;
                    $postArr['detail']['img_num']    = count($imgArr);
                    $post_info_new[]                 = $postArr;
                    $orderArr[]                      = strtotime($v4['created_at']);
                }
                $result = array_merge($result,$post_info_new);
                array_multisort($orderArr, SORT_DESC,$result);
            }
        }
    ...
    PUBLIC function userPms(){
        ...
        $sql = "SELECT max(id) AS id ,relation_id,0 as askid,0 AS user_name,message AS content,max(created_at) AS created_at,0 AS img ,type,4 AS otype,count(*) as num,is_delete FROM {{%pms}} WHERE receive_id = '$userId' AND type = 4 
                UNION all
                SELECT id,relation_id,0 as askid,0 AS user_name,content,created_at,'photo/admin_user.png' AS img ,100 AS type,type as otype,0 as num,is_delete FROM {{%notice_user}} WHERE user_id = '$userId' AND type != '2'
                UNION all 
                SELECT id ,relation_id,askid,user_name ,content,max(created_at) AS created_at,img ,type, otype,count(*) as num,is_delete from (
                SELECT p.id AS id,c.post_id as relation_id,p.relation_id as askid,u.username AS user_name,p.message AS content,p.created_at,u.img ,p.type,c.type as otype,p.is_delete FROM {{%pms}} AS p
                LEFT JOIN {{%user}} AS u ON p.from_id = u.id 
                LEFT JOIN {{%comment}} AS c ON p.relation_id = c.id WHERE p.receive_id = '$userId' AND p.type != 4 ORDER BY p.id DESC) a GROUP BY a.type,a.relation_id,a.content
                UNION all 
                SELECT ns.id AS id,relation as relation_id,0 as askid,a.username AS user_name,ns.content,ns.created_at,'photo/admin_user.png' AS img,99 AS type, type as otype ,0 as num,0 as is_delete
        ...
        if($num > 0){
            ...
            *
            * 字段type ：1是回复帖子 2是点赞 4是系统回复用户 5是收到提问 6 提问消息回复 7二级回复评论 8为后台指定用户一对一的发送消息 9问答评论点赞 10 话题帖子点赞 99（notice_system）,100是系统推送（notice_user）
            * 对应客户端 type ：1 用户消息 2 系统消息 3 提问消息 4 提问消息回复 5用户回复评论 6问答评论点赞 7话题帖子点赞 
            * (以下$type_arr 键名对应字段type，键值对应客户端type)
            * 对应客户端 otype ：0系统消息 1 产品 2 文章 3评论 4后台回复消息 5视频 6话题
            */
            $type_arr = ['1'=>'5','2'=>'1','4'=>'2','5'=>'3','6'=>'4','7'=>'5','8'=>'2','9'=>'6','10'=>'7','99'=>'2','100'=>'2'];
            $no_merge = [1,4,5,6,7,8,100];//表示这些类型的数据不需要合并获取，需要合并是是点赞类型
            ...
            if(!empty($result)){
                foreach ($result as $key => $value) {
                    ...
                    $result[$key]['content']        = Functions::strIsImg($value['content']); //不赋值给$value是为了防止漏掉其他类型

                    if(!in_array($value['type'],$no_merge) && $value['num'] > 1){
                    ...
                    if($value['type'] == 4 || $value['type'] == 8){
                        ...
                        if($value['type'] == 4 && $value['num'] > 1){
                            $chat_sql   = "SELECT * FROM {{%pms}} WHERE id = '$value[id]'";
                            $chat_data  = Yii::$app->db->createCommand($chat_sql)->queryOne();
                            $result[$key]['content']    = Functions::strIsImg($chat_data['message']);
                            $result[$key]['is_delete']  = $chat_data['is_delete'];
                        }
                        $result[$key]['content']    =  $value['type'] == 4 ? '回复了你的意见反馈：'.$result[$key]['content'] : $result[$key]['content'];
                        $result[$key]['otype']      =  $value['type'] == 4 ? '4' : '0';//121版新增判断
                    }
                    ...
                    if($value['type'] == 2){//pms表2为评论点赞,现在需要细分
                        switch ($value['otype']) {
                            case '3':
                                $result[$key]['otype'] = '5';//1.2.2 添加视频点赞
                                break;
                            case '4':
                                $result[$key]['otype'] = '6';//1.2.3 添加话题的评论点赞
                                break;
                            default:
                                $result[$key]['otype'] = $value['otype'];
                                break;
                    ...
                    if($value['type'] == 1 || $value['type'] == 10){
                        $result[$key]['otype'] = '6';//1.2.3 话题类型 注意：跟上面的‘话题的评论点赞’不同，这个type=10 为话题的点赞
                    }
    ...
    */
    PUBLIC function userReply()
    {
        ...
        $sql = "SELECT C.id AS cid,C.first_id AS id,C.created_at,PD.product_name AS content , 1 AS type,C.post_id as product_id FROM {{%comment}} C LEFT JOIN {{%product_details}} PD ON C.post_id = PD.id WHERE C.type = 1 AND C.user_id = '$userId' AND C.parent_id != 0 AND C.status = 1
                UNION 
                SELECT C.id AS cid,C.first_id AS id,C.created_at,AE.title AS content , 2 AS type,0 AS product_id FROM {{%comment}} C LEFT JOIN {{%article}} AE ON C.post_id = AE.id WHERE C.type = 2 AND C.user_id = '$userId' AND C.parent_id != 0 AND C.status = 1
                UNION 
                SELECT 0 AS cid,AR.askid AS id,AR.add_time AS created_at,AK.subject AS content  ,3 AS type, AK.product_id FROM {{%ask_reply}} AR LEFT JOIN {{%ask}} AK ON AR.askid = AK.askid WHERE AR.user_id = '$userId'
                UNION
                SELECT C.id AS cid,C.first_id AS id,C.created_at,V.title AS content , 4 AS type,C.post_id as product_id FROM {{%comment}} C LEFT JOIN {{%video}} V ON C.post_id = V.id WHERE C.type = 3 AND C.user_id = '$userId' AND C.parent_id != 0 AND C.status = 1
                UNION
                SELECT C.id AS cid,C.first_id AS id,C.created_at,P.content , 5 AS type,C.post_id as product_id FROM {{%comment}} C LEFT JOIN {{%post}} P ON C.post_id = P.id WHERE C.type = 4 AND C.user_id = '$userId' AND C.status = 1";
        ...
        if(isset($type) && $type == 1){
            ...
            if(!empty($user_ids)){
                ...
                if(!empty($receive_arr)){
                    foreach($receive_arr as $k => $v){
                        ReplyFunctions::reply($user_id,$askid,ReplyFunctions::$USER_ASK,$subject,$v,$askid);
                    }
                }
            ...
            if(!empty($ask_info)){
                ...
                if($ask_info['user_id'] != $user_id){
                    ...
                    ReplyFunctions::reply($user_id,$askid,ReplyFunctions::$USER_ASK_REPLY,$reply,'',$msg['id']);
                    // 问题得到评论时的推送
    ...
    **/
    PUBLIC function userProduct(){
        ...
        $order      = isset(self::$data['order']) ? intval(self::$data['order']) : 0;
        $time       = strtotime(date('Y-m-d'));

        $whereStr   = "user_id = '$userId'";
        $orderBy    = $order  ? 'expire_time ASC' : 'add_time DESC';
        ...
        if($num > 0){
            ...
            $selectSql = "SELECT up.*,b.rule FROM {{%user_product}} AS up LEFT JOIN {{%brand}} AS b ON up.brand_id = b.id WHERE $whereStr ORDER BY $orderBy LIMIT $pageMin,$pageSize";
            $result    = Yii::$app->db->createCommand($selectSql)->queryAll();
            foreach ($result as $key => $value) {
                $result[$key]['img']  = Functions::get_image_path($value['img']);
                $result[$key]['has_rule']  = $value['rule'] > 0 ? 1 : 0;
                $remain_time = $value['expire_time']+86399-time();
                $result[$key]['remain_days']  =  $remain_time > 0 ? ceil($remain_time/86400) : -1;
                unset($result[$key]['rule']);
            }
        }
        //消息设为已读
        $readSql = "UPDATE {{%notice_user}} SET status = '1' WHERE user_id = '$userId' AND type = '2'";
    ...
    */
    PUBLIC function searchQuery(){
        ...
        $return = [
            'status'    =>  '1',
            'msg'       =>  ['brand' => $data['brand'],'component'=>$data['component'], 'product' => $data['data'],'rank'=>$rankinfo]
        ];
    ...
    */
    PUBLIC function addAskLike()
    {
        ...
        if($userId != $replyInfo['user_id']) {
            //点赞消息
            ReplyFunctions::reply($userId,$replyInfo['askid'],ReplyFunctions::$USER_ASK_REPLY_LIKE,'',$replyInfo['user_id'],$askReplyId);
        }
    ...
    * 添加清单产品
    * @para int    $userId          用户ID
    * @para int    $inventoryId     清单ID
    * @para int    $productId       产品ID
    * @return json 成功返回信息，失败返回错误
    */
    PUBLIC function addInventoryProduct()
    {
        //验证
        $requiredParameter  = array('inventoryId','productId','user_id');
        $this->completeParameter($requiredParameter);

        //查询
        $userId         = intval(self::$data['user_id']);
        $productId      = intval(self::$data['productId']);
        $inventoryId    = intval(self::$data['inventoryId']);
        $desc           = isset(self::$data['desc']) ? Functions::checkStr(self::$data['desc']) : '';
        //判断帐号是否正常
        $this->checkUserStatus($userId);

        //判断清单是否存在
        $inventoryIdInfo = ApiFunctions::getInventory($inventoryId,$userId);

        if(!$inventoryIdInfo){
            $return =  ['status' => -24, 'msg' => self::$ERROR['-24']];
            echo json_encode($return);die;
        }
        //判断是否已加入清单
        $isExsit = ApiFunctions::isExistInventoryProduct($inventoryId,$productId);

        if($isExsit){
            $return =  ['status' => -25, 'msg' => self::$ERROR['-25']];
            echo json_encode($return);die;
        }

        $return   = ApiFunctions::addInventoryProduct($inventoryId,$productId,$desc);
    ...
    * 添加清单
    * @para int    $userId          用户ID
    * @para string $title           标题
    * @para string $picture         图片
    * @return json 成功返回信息，失败返回错误
    */
    PUBLIC function addInventory()
    {
        //验证
        $requiredParameter  = array('title','user_id');
        $this->completeParameter($requiredParameter);

        //查询
        $userId    = intval(self::$data['user_id']);
        $productId = isset(self::$data['productId']) ? intval(self::$data['productId']) : '';
        $picture   = isset(self::$data['picture'])   ? Functions::checkStr(self::$data['picture']) : '';
        $desc      = isset(self::$data['desc'])      ? Tools::userTextEncode(self::$data['desc'],1) : '';
        $title     = Tools::userTextEncode(self::$data['title'],1) ;
        //判断帐号是否正常
        $this->checkUserStatus($userId);

        //添加清单
        $return    = ApiFunctions::addInventory($userId,$title,$picture);
        if($return['status'] != 1) {
            $return =  ['status' => 0, 'msg' => $return['msg']];
            echo json_encode($return);die;
        }

        $inventoryId = $return['msg'];
        if($inventoryId){
            if($productId){
                //判断是否已加入清单
                $isExsit = ApiFunctions::isExistInventoryProduct($inventoryId,$productId);
                ...
                ApiFunctions::addInventoryProduct($inventoryId,$productId,$desc);
            }
            $data = ['status' => 1, 'msg' => self::$ERROR['1'],'inventoryId' => $inventoryId];
        }else{
            $data = ['status' => -200, 'msg' => self::$ERROR['0']];
        }
    ...
    */
    PUBLIC function userInventoryList()
    {
        ...
        $sql_count  = "SELECT COUNT(*) AS num FROM {{%user_inventory}} WHERE status = '1' AND user_id = '$userId'";
        $num        = Yii::$app->db->createCommand($sql_count)->queryScalar();
        //查询
        $inventoryList = ApiFunctions::userInventoryList($userId,$page,$pageSize);

        $data = ['status' => 1, 'msg' => $inventoryList,'pageTotal' => $num, 'pageSize'=> $pageSize];
    ...
    /**
    * 删除用户清单
    * @para int    $invent_id          清单id
    * @return json 成功返回信息，失败返回错误
    */
    PUBLIC function delInventory()
    {
        //验证
        $requiredParameter  = array('inventoryId','user_id');
        $this->completeParameter($requiredParameter);
        //参数
        $inventoryId   = intval(self::$data['inventoryId']);
        $userId        = intval(self::$data['user_id']);
        ...
        $sql    = "DELETE  FROM {{%user_inventory}} WHERE id = '$inventoryId'";
        Yii::$app->db->createCommand($sql)->execute();

        $sql    = "DELETE  FROM {{%user_inventory_product}} WHERE invent_id = '$inventoryId'";
        Yii::$app->db->createCommand($sql)->execute();
    ...
    * @return json 成功返回信息，失败返回错误
    */
    PUBLIC function userInventoryDetail()
    {
        //验证
        $requiredParameter  = array('invent_id');
        $this->completeParameter($requiredParameter);
        //参数
        $invent_id  = intval(self::$data['invent_id']);
        $pageSize   = isset(self::$data['pageSize']) ? intval(self::$data['pageSize']) : 10;
        $page       = isset(self::$data['page'])     ? intval(self::$data['page']) : 1;
        $pageMin    = ($page - 1) * $pageSize;
        $res = [];

        $sql_count  = "SELECT COUNT(*) AS num FROM {{%user_inventory_product}} WHERE invent_id = '$invent_id'";
        $num        = Yii::$app->db->createCommand($sql_count)->queryScalar();

        $sql    = "SELECT uip.*,pd.product_name,pd.product_img,pd.price,pd.form,pd.star FROM {{%user_inventory_product}} AS uip
                    LEFT JOIN {{%product_details}} AS pd ON pd.id = uip.product_id
                    WHERE invent_id = '$invent_id'
                    ORDER BY `order` ASC,add_time DESC
                    LIMIT $pageMin,$pageSize";
        $res        = Yii::$app->db->createCommand($sql)->queryAll();
        if($res){
            foreach ($res as $key => $value) {
                $res[$key]['product_img']   = Functions::get_image_path($value['product_img'],1);
                $res[$key]['desc']          = !empty($value['desc']) ? Tools::userTextDecode($value['desc'],1) : '';
            }
        }
        $data = ['status' => 1, 'msg' => $res,'pageTotal' => $num, 'pageSize'=> $pageSize];
        return  json_encode($data);
    ...
    * @return json 成功返回信息，失败返回错误
    */
    PUBLIC function operateInDetail()
    {
        //验证
        $requiredParameter  = array('uip_id','type');
        $this->completeParameter($requiredParameter);
        //参数
        $uip_id   = intval(self::$data['uip_id']);
        $type     = intval(self::$data['type']);
        if($type == 1){
            $requiredParameter  = array('desc');
            $this->completeParameter($requiredParameter);
            $desc      = isset(self::$data['desc'])      ? Tools::userTextEncode(self::$data['desc'],1) : '';

            $sql    = "UPDATE {{%user_inventory_product}} SET `desc`='$desc' WHERE id = '$uip_id'";
            Yii::$app->db->createCommand($sql)->execute();
        }else{
            ApiFunctions::updateInventoryPicture($uip_id);
            $sql    = "DELETE  FROM {{%user_inventory_product}} WHERE id = '$uip_id'";
            Yii::$app->db->createCommand($sql)->execute();
        }
    ...
    /**
    * 操作用户清单详情的顺序
    * @para int    $uip_ids          清单uip_ids
    * @return json 成功返回信息，失败返回错误
    */
    PUBLIC function operateInOrder()
    {
        //验证
        $requiredParameter  = array('uip_ids');
        $this->completeParameter($requiredParameter);
        //参数
        $uip_ids     = Functions::checkStr(self::$data['uip_ids']);
        $uip_ids_arr = explode(',', $uip_ids);
        $sql = 'UPDATE {{%user_inventory_product}} SET `order` = CASE id ';
        if(is_array($uip_ids_arr)){
            foreach ($uip_ids_arr as $key => $value) {
                $sql .= ' WHEN '.$value.' THEN '.$key;
            }
            $sql .= ' END WHERE '.Functions::db_create_in($uip_ids_arr,'id');
            Yii::$app->db->createCommand($sql)->execute();
    ...
    PUBLIC function videoInfo() {
        ...
        if(!$videoInfo) {
            $data = ['status' => -26, 'msg' => self::$ERROR['-26']];
            return  json_encode($data);
        }
        //视频点击数
        Functions::videoClick($id);
        $data = ['status' => '1','msg' => $videoInfo];
        return  json_encode($data);
    }
    ...
    PUBLIC function buyToday(){
        ...
        $sqlCount    = "SELECT COUNT(*) AS num  FROM {{%product_bonus}} WHERE status = 1 AND is_off = 1 AND (end_date >= '{$date}' OR bonus_link = ' ')";
        $num    = Yii::$app->db->createCommand($sqlCount)->queryScalar();

        //获取商品id
        $pageMin= ($page - 1) * $pageSize;
        $productSql    = "SELECT B.id,B.goods_id,B.goods_link,B.bonus_link,B.price,B.start_date,B.end_date,B.sort,P.comment_num FROM {{%product_bonus}} as B
                          LEFT JOIN {{%product_details}} P ON P.id = B.product_id WHERE B.status = 1 AND B.is_off = 1 AND
                          (B.end_date >= '{$date}' OR B.bonus_link = ' ') ORDER BY B.sort DESC, B.is_manual ASC, B.price DESC, P.comment_num DESC LIMIT $pageMin,$pageSize";
        $productBonusArr   = Yii::$app->db->createCommand($productSql)->queryAll();

        $data = [];
        $messageArr = [];
        foreach ($productBonusArr as $productBonus){
            $data[] = $productBonus['goods_id'];
            $messageArr["{$productBonus['goods_id']}"] = [
                'goods_link'=>$productBonus['goods_link'],
                'bonus_link'=>$productBonus['bonus_link'],
                'price'=>$productBonus['price'],
                'start_date'=>$productBonus['start_date'],
                'end_date'=>$productBonus['end_date'],
            ];
        }
        ...
        if($results){
            foreach ($results as $key=>$result){
                if($messageArr["{$result['num_iid']}"]['bonus_link']){
                    $results[$key]['link'] = $messageArr["{$result['num_iid']}"]['bonus_link'];
                    $results[$key]['price'] = $messageArr["{$result['num_iid']}"]['price'];
                }else{
                    $results[$key]['link'] = $messageArr["{$result['num_iid']}"]['goods_link'];
                }
            }
        }

        return  json_encode(['status' => 1, 'msg' => $results,'pageTotal' => $num, 'pageSize'=> $pageSize]);
    }
    /**
     * [getAppMenu 单独获取菜单接口]
     * @return [type] [description]
    ...
    PUBLIC function  getStartup(){
        $time   = time();
        //查询
        $sql    =  "SELECT id,type,url,img,relation_id,start_time,end_time 
                    FROM {{%startup}} 
                    WHERE status = '1' AND start_time <= '$time' AND end_time >= '$time'";
        $info   = Yii::$app->db->createCommand($sql)->queryOne();

        if($info){
            $info['img'] = Functions::get_image_path($info['img']);
        }else{
            $info = (object)[];
        }

        return  json_encode(['status' => 1, 'msg' => $info]);
    }
    /**
     * [topicInfo 话题详情]
     * @return [type] [description]
    ...
    PUBLIC function topicInfo(){
        ...
        if(!$topicInfo) {
            $data = ['status' => -27, 'msg' => self::$ERROR['-27']];
            return  json_encode($data);
        }
        $sql_count  = "SELECT COUNT(*) AS num FROM {{%post}} WHERE status = '1' AND topic_id = '$id'";
        $num        = Yii::$app->db->createCommand($sql_count)->queryScalar();
        //查询
        $params     = ['id' => $id,'userId' => $userId,'orderBy' => '1', 'page' => $page,'pageSize' => $pageSize];
        $postList   = ApiFunctions::postList($params);
        $topicInfo['postList'] = $postList;

        $data = ['status' => '1','msg' => $topicInfo,'pageTotal' => $num, 'pageSize'=> $pageSize];
        return  json_encode($data);
    }
    /**
    * 话题贴列表
    ...
    */
    PUBLIC function postList()
    {
        ...
        $id         = intval(self::$data['id']);
        $userId     = isset(self::$data['user_id'])   ? intval(self::$data['user_id']) : 0;
        $orderBy    = isset(self::$data['orderBy'])  ? intval(self::$data['orderBy']) : 1;
        $pageSize   = isset(self::$data['pageSize']) ? intval(self::$data['pageSize']) : 10;
        $page       = isset(self::$data['page'])     ? intval(self::$data['page']) : 1;
        $pageMin    = ($page - 1) * $pageSize;
        ...
        $num        = Yii::$app->db->createCommand($sql_count)->queryScalar();
        //查询
        $params     = ['id' => $id,'userId' => $userId,'orderBy' => $orderBy, 'page' => $page,'pageSize' => $pageSize];
        $postList   = ApiFunctions::postList($params);

        $data = ['status' => 1, 'msg' => $postList,'pageTotal' => $num, 'pageSize'=> $pageSize];
    ...
    PUBLIC function postInfo() {
        ...
        $id         = isset(self::$data['id']) ? intval(self::$data['id']) : '';
        $userId     = isset(self::$data['user_id']) ? intval(self::$data['user_id']) : '';
        $postInfo   = Functions::getPostInfo($id,$userId);

        if(!$postInfo) {
            $data = ['status' => -28, 'msg' => self::$ERROR['-28']];
            return  json_encode($data);
        }
        //排序
        ...
        if(!empty($commentList)){
            foreach ($commentList as $key => $value) {
                $commentArr[$key]   =  Functions::getCommentInfo($value['id'],$userId,1);
                $reply = Functions::getCommentReply($value['id']);
                if($reply)$commentArr[$key]['reply'] =  $reply;
                unset($reply);
            }
        }
        $postInfo['commentList']  = $commentArr;

        $data = ['status' => '1','msg' => $postInfo];
        return  json_encode($data);
    }
    /**
     * [addPost 发话题帖]
    ...
    PUBLIC function addPost() {
        //验证
        $requiredParameter  = array('id','content');
        $this->completeParameter($requiredParameter);
        //参数
        $id         = isset(self::$data['id']) ? intval(self::$data['id']) : '';
        $userId     = isset(self::$data['user_id']) ? intval(self::$data['user_id']) : '';
        $ratio      = isset(self::$data['ratio']) ? floatval(self::$data['ratio']) : '';
        $picture    = isset(self::$data['picture']) ? Functions::checkStr(self::$data['picture']) : '';
        $attachment = isset(self::$data['attachment']) ? self::$data['attachment'] : '';
        $content    = Functions::checkStr(self::$data['content']);
        ...
        //入库
        $sql      = "INSERT INTO {{%post}} (user_id,topic_id,picture,ratio,content) VALUES ('$userId','$id','$picture','$ratio','$content')";

        $return    = Yii::$app->db->createCommand($sql)->execute();
        $newPostId =  Yii::$app->db->getLastInsertId();

        //入库附件
        if($attachment){
            foreach ($attachment as $attachKey => $attachVal) {
                $attachmentSql  = "INSERT INTO {{%attachment}} (cid,type,uid,attachment,dateline) VALUES ('$newPostId','2','$userId','$attachVal','$time')";
                Yii::$app->db->createCommand($attachmentSql)->execute();
                $attachmentList[] =  Functions::get_image_path($attachVal);
                unset($attachmentSql);
            }
        ...
        if($newPostId){
            $data = ['status' => 1, 'msg' => ['postId' => $newPostId,'attachment' => $attachmentList]];
        }else{
            $data = ['status' => 0, 'msg' => self::$ERROR['0']];
        }

        //统计话题帖子评论数
        $TopicSql  = "UPDATE {{%topic}} T SET T.post_num = (SELECT COUNT(*) FROM {{%post}} P WHERE P.topic_id = {$id} AND P.status = 1)  WHERE T.id = {$id}";
    ...
    PUBLIC function addPostLike(){
        //验证
        $requiredParameter  = array('postId');
        $this->completeParameter($requiredParameter);

        //查询
        $userId     = intval(self::$data['user_id']);
        $postId     = intval(self::$data['postId']);
        $referer    = strtolower(self::$data['from']) == 'ios' ? 'ios' : 'android';
        //判断帐号是否正常
        $this->checkUserStatus($userId);

        //判断帖子或评论是否存在
        $infoSql     = "SELECT user_id,topic_id FROM {{%post}} WHERE id = '$postId'";
        $commentInfo = Yii::$app->db->createCommand($infoSql)->queryOne();

        if(!$commentInfo){
            $return =  ['status' => -28, 'msg' => self::$ERROR['-28']];
            echo json_encode($return);die;
        }
        //判断是否已点过赞
        $selectSql   = "SELECT id FROM {{%post_like}} WHERE user_id = '$userId' AND post_id = '$postId'";
        $isExsit     = Yii::$app->db->createCommand($selectSql)->queryScalar();

        if($isExsit){
            $sql    = "DELETE  FROM {{%post_like}} WHERE id = '$isExsit'";
            $rows   = Yii::$app->db->createCommand($sql)->execute();
        }else{
            $sql    = "INSERT INTO {{%post_like}} (user_id,topic_id,post_id,referer) 
                        VALUES('$userId','$commentInfo[topic_id]','$postId','$referer')";
            $rows   = Yii::$app->db->createCommand($sql)->execute();
        }

        //修改点赞数
        $updateSql  = "UPDATE {{%post}} P SET P.like_num = (SELECT COUNT(*)  FROM {{%post_like}}  WHERE  post_id = '$postId') WHERE P.id = '$postId'";

        Yii::$app->db->createCommand($updateSql)->execute();

        if($rows){
            if(!$isExsit && ($userId != $commentInfo['user_id'])){
                //点赞消息
                ReplyFunctions::reply($userId,$postId,ReplyFunctions::$POST_LIKE,'',$commentInfo['user_id']);
            }
            $data = ['status' => 1, 'msg' => self::$ERROR['1']];
        }else{
            $data = ['status' => -200, 'msg' => self::$ERROR['0']];
