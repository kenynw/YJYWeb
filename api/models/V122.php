...
class V122 extends Model{
    ...
    */
    PUBLIC function addRemind(){
        //验证
        $requiredParameter  = array('product','seal_time','overdue_time');
        $this->completeParameter($requiredParameter);
    ...
    */
    ...
    PUBLIC function userPms(){
        ...
        if($num > 0){
            ...
            * 对应客户端 otype ：0系统消息 1 产品 2 文章 3评论 4后台回复消息 5视频
            */
            ...
            if(!empty($result)){
                foreach ($result as $key => $value) {
                    ...
                    $result[$key]['otype']  = $value['type'] == 2 && $value['otype'] == 3 ? '5' : $value['otype'];//1.2.2 添加视频点赞
    ...
    */
    PUBLIC function userReply()
    {
        ...
        $sql = "SELECT C.first_id AS id,C.created_at,PD.product_name AS content , 1 AS type,C.post_id as product_id FROM {{%comment}} C LEFT JOIN {{%product_details}} PD ON C.post_id = PD.id WHERE C.type = 1 AND C.user_id = '$userId' AND C.parent_id != 0 AND C.status = 1
                ...
                UNION
                SELECT C.first_id AS id,C.created_at,V.title AS content , 4 AS type,C.post_id as product_id FROM {{%comment}} C LEFT JOIN {{%video}} V ON C.post_id = V.id WHERE C.type = 3 AND C.user_id = '$userId' AND C.parent_id != 0 AND C.status = 1";
    ...
    */
    ...
    PUBLIC function buyToday(){
        ...
        $sqlCount    = "SELECT COUNT(*) AS num  FROM {{%product_bonus}} WHERE status = 1 AND is_off = 1";
        $num    = Yii::$app->db->createCommand($sqlCount)->queryScalar();

        //获取商品id
        $pageMin= ($page - 1) * $pageSize;
        $productSql    = "SELECT id,goods_id,goods_link,bonus_link,price,start_date,end_date FROM {{%product_bonus}} 
                          WHERE status = 1 AND is_off = 1 ORDER BY updated_at DESC, id ASC LIMIT $pageMin,$pageSize";
        ...
        if($results){
            foreach ($results as $key=>$result){
                if($messageArr["{$result['num_iid']}"]['bonus_link'] &&  $date <= $messageArr["{$result['num_iid']}"]['end_date']
                ){
