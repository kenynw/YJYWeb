...
class V110 extends Model{  
    ...
    */
    PUBLIC function addComment(){
        ...
        if($return){
            ...
            $data = ['status' => 1, 'msg' => self::$ERROR['1'],'commentId' => $newcommentId,'attachment' => $attachmentPath];
        }else{
    ...
    */
    PUBLIC function userInfo(){
        ...
        //查询
        $sql        = "SELECT U.id,U.username,U.mobile,U.email,U.img,U.img_state,U.sex,U.birth_year,U.birth_month,U.birth_day,U.province,U.city,U.status,U.rank_points,S.dry,S.tolerance,S.pigment,S.compact,S.skin_name,S.add_time
    ...
    */
    PUBLIC function userFeedback(){
        //验证
        $requiredParameter  = array('username','content');
        $this->completeParameter($requiredParameter);
    ...
    public function thirdLogin()
    {
        ...
        try {
            ...
            if(!empty($userId)){
                ...
                //表写锁定
                Yii::$app->db->createCommand("lock tables {{%third_login}} WRITE")->execute();
    ...
    */
    ...
    PUBLIC function userPms(){
        ...
        $sql = "SELECT id,0 as relation_id,0 as askid,0 AS user_name,content,created_at,'photo/admin_user.png' AS img ,100 AS type,0 as otype,0 as num FROM {{%notice_user}}  WHERE user_id = '$userId' AND type != '2'
                UNION all 
                ...
                foreach ($result as $key => $value) {
                    ...
                    if($value['type'] == 99){
                        ...
                        switch ($value['otype']) {//notice_system表1为过期提醒，2为H5，3为文章,4为产品，0为常规
                            case '2':
                                $result[$key]['content']        = $value['content'].$value['relation_id'];
                    ...
                    $result[$key]['type']           = $value['type'] >= 4 ? '2' : '1'; //4是系统回复用户 5是收到提问 99,100是系统推送 2是点赞
    ...
    */
    PUBLIC function subAsk()
    {
        ...
        if(isset($type) && $type == 1){
            $requiredParameter  = array('product_id','username','product_name','content');
            $this->completeParameter($requiredParameter);
            ...
            $username       = isset(self::$data['username'])        ? Tools::userTextEncode(self::$data['username'],1) : '' ;
    ...
    */
    PUBLIC function searchQuery(){
        ...
        if($data['status'] != 1) return json_encode(['status' => 1,'msg' => ['categoryList'=>$categoryList,'effectList'=>$effectList,'brand' => (object)[],'product' => []],'pageTotal' => 0 , 'page'=>$page]);
