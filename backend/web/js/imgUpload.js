...
$(function(){
  ...
  $(".js-key-show").on('change','#file_upload', function() {
	var imgFilename = $("#uploadbtn").attr('data-imgFilename');	
	var controllerId = $("#uploadbtn").attr('data-controllerId');	
      ...
      for(var i=0;i<this.files.length;i++){
        ...
        if (/\/(?:gif)/i.test(file.type)) {
          ...
          reader.onload = function(e) {
            $.post(
              "/"+controllerId+"/upload-img?imgFilename="+imgFilename,
  ...
  function compress(img) {
    ...
    console.log(ctx);
    //进行最小压缩
    var ndata = canvas.toDataURL('image/jpeg', 0.99);
  ...
  function upSuccess(data) {
    if(data.filename){
      ...
      var img_obj='<li class="img_file_li" id="'+id+'"><a><input type="text" id="post-views_num" class="form-control" name="img[pic_list][]" value="'+data.filename+'" style="display:none;"><img src="'+showUrl+data.filename+'" data="'+showUrl+data.filename+'" style="width:100px;height:100px;" alt="" class="images" id="image'+len+'" data-key ="'+len+'"><i class="icon-close" onclick=removeImg($("#'+id+'"))></i></a></li>';
