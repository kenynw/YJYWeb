...
<div class="comment-index">
    ...
    <?= GridView::widget([
        ...
        'columns' => [
            ...
            [
                'attribute' => 'comment',
                'contentOptions'=>["style"=>"max-width:40%;word-break:break-all;white-space: normal;"],
                'format' => 'raw',
                'value' => function($model) use ($faces){
                    //显示评论表情
        ...
        success : function(data) {
		    ...
		    for(var i = 0;i<length;i++) {
                ...
                if (result[i]['first_id'] == result[i]['parent_id']) {
                    ...
                    //二级以上回复
                    html += '<div style="width:100%;padding:0 0 5px 0;"><span style="color:'+color1+';">'
                            +result[i]['username']+'</span> 回复<span style="color:'+color2+'">'+result[i]['parent_username']+'</span>：'
                            +result[i]['comment']+'&emsp;<span style="color:#AEAEAE;font-size:12px;"><br>'
                            +result[i]['created_at']
                            +'</span>&nbsp;</div>'+button;
                }
