...
empty($type) && !isset($user_id) ? $this->title = '评论管理' : false;
empty($type) && !isset($user_id) ? $this->params['breadcrumbs'][] = $this->title : false;

//评论列表(文章详情页，产品详情页，用户详情页)
$data_id = isset($data_id) ? $data_id : "";
$user_id = isset($user_id) ? $user_id : "";

$search = Yii::$app->request->queryParams;
//产品详情/文章详情
empty($type) ? false : $search['CommentSearch']['type'] = $type;
if (!empty($type)) {
    if (!empty($data_id)) {
        $search['CommentSearch']['post_id'] = $data_id;
    }
...
//用户详情
    $type = '';
    if (!empty($user_id)) {
        $search['CommentSearch']['user_id'] = $user_id;
        $searchModel = new CommentSearch();
        $dataProvider = $searchModel->search($search);
    }
}
...
<div class="comment-index" name="comment" id="comment">
    ...
    <?php if(!empty($type)) { ?>
        ...
        <?php if($data_id){
            echo Html::a('<i class="glyphicon glyphicon-plus"></i> 创建评论','javascript:void(0)', ['class' => 'btn btn-success create-comment','data-target'=>'#comment-modal','data-toggle'=>'modal','data-type'=>$type,'data-post-id'=>$data_id,'data-first-id' => '0','data-parent-id' => '0','data-jump-url'=>$jump_url,'data-modal-type' => '0','style'=>'float:left;' ]);
        } ?>
        </div>
...
    <?= GridView::widget([
        ...
        'options' => ['class' => 'grid-comment'],
        "id" => 'grid-comment',
        "rowOptions" => function ($model, $key, $index, $grid) {
            return $model->status == 1 ? '' : ['style' => 'background-color:#eff5fb'];
        },
        'showFooter' => true,
        'columns' => [
            [
            ...
            [
                ...
                'format' => 'raw',
                //评论列表
                'visible' => $type == '' && !empty($user_id) ? '1' : '0',
...
//                         $str = Html::a('查看原评论','javascript:void(0)',['style'=>'font-size:10px;color:blue;','data-container'=>'body','data-toggle'=>'popover','data-placement'=>'right','data-html'=>'true','data-content'=>$content,'data-trigger'=>'hover']);
            ...
            [
                ...
                'visible' => $type == '2' ? '0' : '1',
                'value' => function($model){
                    if ($model->type == '1' && empty($model->first_id) && empty($model->parent_id)) {
                        //                         if ($status = $model->status == 0) {
                        //                             return '<button class="btn btn-xs">已删除</button>';
                        //                         } else {
                        if($model->is_digest == 1){
                            return $model->status == 1 ? "<button class='btn btn-success btn-xs btnstatus' data-status='1' data-type='is_digest' data-id='".$model->id."'>精华</button>" : "<button class='btn btn-xs btnstatus' data-status='1' data-type='is_digest' data-id='".$model->id."' disabled='disabled'>精华</button>";
                        }else{
                            return "<button class='btn btn-xs btnstatus' data-status='0' data-type='is_digest' data-id='".$model->id."'>默认</button>";
                        }
                        //                         }
                    } else {
                        return '';
            ...
            [
                ...
                'value' => function ($model) {
                    $return = $model->star.'星';
                    if (!empty($model->first_id) || !empty($model->parent_id) || $model->type != '1') {
                        $return = '';
                    }
                    return $return;
                },
            ...
            [
                ...
                'value' => function($model){
                    $return = '';
                    if ($model->type == '1') {
                        $return = Html::a(Comment::getPostId($model->type, $model->post_id), ['product-details/view','id'=> $model->post_id]);
                    } elseif ($model->type == '2') {
                        $return = Html::a(Comment::getPostId($model->type, $model->post_id), ['article/view','id'=> $model->post_id]);
                    } elseif ($model->type == '3') {
                        $return = Html::a(Comment::getPostId($model->type, $model->post_id), ['video/view','id'=> $model->post_id]);
                    }
                    return $return;
                },
            ],            
...
$(document).on('click', '.delete_alls', function () {
    var id = $("#grid-comment").yiiGridView("getSelectedRows");
...
$("input[type='checkbox']").on('click',function(){
    if ($("#grid-comment input[type='checkbox']").is(':checked')) {
        $('#grid-comment .delete_alls').attr('disabled',false);
        $('#grid-comment .delete_alls').addClass('btn-danger');
    } else {
        $('#grid-comment .delete_alls').attr('disabled','disabled');
        $('#grid-comment .delete_alls').removeClass('btn-danger');
    }
})
$("input[name='id_all']").on('click',function(){
    $("#grid-comment tbody input[type='checkbox']").each(function(){
        if($(this).is(':checked')){
            $('#grid-comment .delete_alls').attr('disabled','disabled');
            $('#grid-comment .delete_alls').removeClass('btn-danger');
        } else {
