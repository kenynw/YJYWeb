				...
				for (;start <= end; start++) {
					if(start <= args.pageCount && start >= 1){
						if(start != args.current){
							if(start == args.pageCount && args.pageCount >= 5){
								obj.append('<a href="javascript:;" class="tcdNumber lastPage">尾页</a>');
							}else{
								obj.append('<a href="javascript:;" class="tcdNumber">'+ start +'</a>');
							}
						}else{
							if(start == args.pageCount && args.pageCount >= 5){
								obj.append('<span class="current lastPage">尾页</span>');
							}else{
...
//				});
				...
				obj.on("click","a.prevPage",function(){
					...
					if(isNaN(current)){
						ms.fillHtml(obj,{"current":args.pageCount-1,"pageCount":args.pageCount});
						if(typeof(args.backFn)=="function"){
							args.backFn(args.pageCount-1);
						}
					}else{
