...
class Wechat
{
	...
	*/
    ...
    public function checkAuths($appid='',$appsecret='') {
        ...
        $data = json_decode(file_get_contents($baseUrl."access_token.json"));
    ...
    //获取 jsapi_ticket，并缓存起来
    public function getToken($url="",$type=""){
        ...
        //取数据
        $b = file_get_contents($baseUrl."weixin-token.txt");
        $data = @unserialize(base64_decode($b));
        //未过期，读缓存
        if(!empty($data[$type]) && (($time - $data[$type]['valid_time']) < 6000) ) {
            $access_token = $data[$type]['access_token'];
            $jsapi_ticket = $data[$type]['jsapi_ticket'];
            $valid_time = $data[$type]['valid_time'];
            $valid_time = $time;
	        $noncestr = $time;
	        $timestamp = $time;
	        $and = "jsapi_ticket=".$jsapi_ticket."&noncestr=".$noncestr."&timestamp=".$timestamp."&url=".$url."";
	       	...
	       	$data[$type]['APPID'] = $appid;
	        $data[$type]['access_token'] = $access_token;
	        $data[$type]['jsapi_ticket'] = $jsapi_ticket;
	        $data[$type]['timestamp'] = $timestamp;
	        $data[$type]['noncestr'] = $noncestr;
	        $data[$type]['signature'] = $signature;
	        $data[$type]['valid_time'] = $valid_time;
        }else{
            ...
            $token = json_decode($accesstoken, true); //对JSON格式的字符串进行编码
            $access_token = $token['access_token'];//输出access_token
            ...
            $jsapi_ticket = $jsapi['ticket'];//get JSAPI
	        ...
	        file_put_contents($baseUrl."weixin-token.txt",$content);
        }
        return $data[$type];
    }
    ...
    private function getJsApiTicket() {
        $baseUrl = dirname(Yii::$app->BasePath).'/frontend/web/uploads/';
        $data = json_decode(file_get_contents($baseUrl."jsapi_ticket.json"));

        if (!$this->access_token && !$this->checkAuths()) return false;
    ...
    public function checkAvail($access_token='',$openid=''){
        ...
        if($access_token && $openid){
            ...
            $avail_data = $this->http_get($avail_url);
